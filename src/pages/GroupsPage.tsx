...
import {
  ...
  Building2, Edit, Trash2, MessageCircle, ShoppingCart, Wallet, FileText
} from 'lucide-react';

export default function GroupsPage() {
  const { userProfile, user } = useAuth();
  const { permissions, loading: permissionsLoading, getEffectiveOwnerId } = usePermissions();
            ...
            <div key={c.id} className="p-3 border border-border/40 rounded-xl">
              <div className="flex items-start justify-between">
                ...
                <div className="flex gap-1 flex-shrink-0">
                  ...
                  <Button size="icon" variant="ghost" className="h-7 w-7" onClick={() => { setSelectedCustomer({ id: c.id, name: c.name }); setLedgerOpen(true); }}>
              ...
              {quickPayAmount && parseFloat(quickPayAmount) > 0 && (
                <div className="bg-primary/5 rounded-xl p-3">
                  ...
                  <p className="text-lg font-bold text-foreground">
                    TSh {Math.max(0, (quickPayCustomer.outstanding_balance || 0) - parseFloat(quickPayAmount || '0')).toLocaleString()}
                  </p>
                </div>
              )}
